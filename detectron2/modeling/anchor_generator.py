...
def _create_grid_offsets_nonsquare(size: List[int], stride: List[int], offset: float, device: torch.device):
    grid_height, grid_width = size
    shifts_x = torch.arange(
        offset * stride[1], grid_width * stride[1], step=stride[1], dtype=torch.float32, device=device
    )
    shifts_y = torch.arange(
        offset * stride[0], grid_height * stride[0], step=stride[0], dtype=torch.float32, device=device
    )
...
class ITDDefaultAnchorGenerator(nn.Module):
    ...
    @configurable
    def __init__(self, *, sizes, aspect_ratios, num_features, offset=0.5):
        """
        ...
        Args:
            ...
            num_features (int): Number of feature maps from backbone.
    ...
    def from_config(cls, cfg):
        return {
            ...
            "num_features": cfg.MODEL.ITD_BACKBONE.NUM_FEATURES,
    ...
    def _grid_anchors(self, grid_sizes: List[List[int]], strides: List[List[int]]):
        """
        Returns:
        ...
        for size, stride, base_anchors in zip(grid_sizes, strides, buffers):
            shift_x, shift_y = _create_grid_offsets_nonsquare(size, stride, self.offset, base_anchors.device)
    ...
    def forward(self, features: List[torch.Tensor], input_shape):
        """
        Args:
            features (list[Tensor]): list of backbone feature maps on which to generate anchors.
            input_shape: (Tensor.shape): shape of input
        ...
        for gs in grid_sizes:
           # Round to the smallest power of 2 greater than or equal to n
           curr_stride_v = 1 << (round(input_resolution[0] / gs[0]) - 1).bit_length()
           curr_stride_h = 1 << (round(input_resolution[1] / gs[1]) - 1).bit_length()
           strides.append([curr_stride_v, curr_stride_h])
...
def build_itd_anchor_generator(cfg):
    """
    Built an anchor generator from `cfg.MODEL.ANCHOR_GENERATOR.NAME`.
    """
    anchor_generator = cfg.MODEL.ANCHOR_GENERATOR.NAME
